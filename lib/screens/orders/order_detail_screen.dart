...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (_error != null || _order == null) {
      return Scaffold(
        ...
        appBar: AppBar(backgroundColor: Colors.transparent, elevation: 0,
          leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new_rounded, color: AppColors.darkText), onPressed: () => Navigator.pop(context))),
        body: Center(child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.error_outline, size: 48, color: Colors.grey.shade400),
            const SizedBox(height: 12),
            Text(_error ?? 'Order not found', style: const TextStyle(color: AppColors.warmGray)),
            const SizedBox(height: 16),
            TextButton(onPressed: _load, child: const Text('Retry', style: TextStyle(color: AppColors.coral))),
          ],
        )),
      );
    }
    ...
    final order = _order!;
    final canCancel = [OrderStatus.pending, OrderStatus.confirmed].contains(order.status);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('#${order.id.substring(0, 8).toUpperCase()}',
            style: const TextStyle(color: AppColors.darkText, fontWeight: FontWeight.w700, fontSize: 16)),
        centerTitle: true,
        actions: [
        ...
        child: Column(
          ...
          children: [
            ...
            _Section(
              ...
              child: Column(
                children: [
                  _DetailRow(icon: Icons.location_on_rounded, label: 'Zone', value: order.zoneName),
                  _DetailRow(icon: Icons.home_rounded, label: 'Address', value: order.address),
                  _DetailRow(
                    icon: Icons.calendar_today_rounded,
                    label: 'Date',
                    value: DateFormat('EEEE, MMMM d, y').format(order.scheduledPickupDate),
                  ),
                  _DetailRow(icon: Icons.access_time_rounded, label: 'Time Slot', value: order.scheduledPickupTime),
                  if (order.notes != null)
                    _DetailRow(icon: Icons.notes_rounded, label: 'Notes', value: order.notes!),
                ],
              ),
            ),
            const SizedBox(height: 16),
                          ...
                          child: Row(
                            children: [
                              ...
                              Text('₦${fmt.format(item.total)}',
                                  style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: AppColors.darkText)),
                            ],
                          ),
                        ),
              ...
              child: Column(
                children: [
                  ...
                  if (order.flutterwaveRef != null)
                    _DetailRow(icon: Icons.tag_rounded, label: 'Ref', value: order.flutterwaveRef!),
                  const Divider(height: 20),
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Subtotal', style: TextStyle(color: AppColors.warmGray, fontSize: 13)),
                      Text('₦${fmt.format(order.subtotal)}', style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.darkText)),
                    ],
                  ),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Delivery Fee', style: TextStyle(color: AppColors.warmGray, fontSize: 13)),
                      Text('₦${fmt.format(order.deliveryFee)}', style: const TextStyle(fontWeight: FontWeight.w600, color: AppColors.darkText)),
                    ],
                  ),
                  ...
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      const Text('Total', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15, color: AppColors.darkText)),
                      Text('₦${fmt.format(order.total)}',
                          style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17, color: AppColors.darkText)),
                    ],
                  ),
                ],
              ),
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  child: _cancelling
                      ...
                      : const Text('Cancel Order', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700, fontSize: 15)),
                ),
              ),
            ],
...
class _StatusTimeline extends StatelessWidget {
  ...
  static const _steps = [
    ...
    _Step(OrderStatus.confirmed, 'Confirmed', Icons.check_circle_outline_rounded),
    ...
    if (status == OrderStatus.cancelled) {
      return Row(
        children: [
          ...
          const Text('Order Cancelled', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700, fontSize: 15)),
        ],
      );
    }
              ...
              children: [
                AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: isActive
                        ? [BoxShadow(color: AppColors.coral.withOpacity(0.3), blurRadius: 8, spreadRadius: 2)]
                        : null,
                  ),
                ...
                if (!isLast)
                  AnimatedContainer(
                    ...
                    width: 2,
                    height: 28,
                    color: i < _currentIndex ? AppColors.coral : Colors.grey.shade200,
                  ),
              ],
            ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (isActive)
                    Text('In progress', style: TextStyle(fontSize: 11, color: AppColors.coral.withOpacity(0.8))),
                ],
              ),
            ),
          ],
...
class _Section extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.darkText)),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(
      ...
      children: [
        ...
        SizedBox(width: 72, child: Text(label, style: const TextStyle(color: AppColors.warmGray, fontSize: 13))),
        Expanded(
          child: Text(
            value,
            style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: valueColor ?? AppColors.darkText),
          ),
        ),
      ],
    ),
