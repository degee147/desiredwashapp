...
class WelcomeScreen extends StatelessWidget {
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: AppColors.coral.withOpacity(0.2), blurRadius: 20, offset: const Offset(0, 8))],
                  ),
                  child: const Icon(Icons.local_laundry_service_rounded, color: AppColors.coral, size: 38),
                ),
                const SizedBox(height: 28),
                const Text('Laundry done\nfor you. 🫧',
                    style: TextStyle(fontSize: 38, fontWeight: FontWeight.w800, color: AppColors.darkText, height: 1.1)),
                const SizedBox(height: 14),
                Text('Schedule a pickup in minutes. We wash,\ndry & deliver in Port Harcourt.',
                    style: TextStyle(fontSize: 16, color: AppColors.warmGray, height: 1.5)),
                const Spacer(flex: 2),
                ...
                Row(children: [
                  ...
                  Padding(
                    ...
                    child: Text('or', style: TextStyle(color: AppColors.warmGray)),
                  ),
                  Expanded(child: Divider(color: AppColors.cream)),
                ]),
                const SizedBox(height: 20),
  ...
  void _handleApple(BuildContext context) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Add sign_in_with_apple package to enable')));
  }
}
            ...
            child: Column(
              ...
              children: [
                const Text('Create account',
                    style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.darkText)),
                const SizedBox(height: 6),
                Text('Get started with DesiredWash 🫧',
                    style: TextStyle(fontSize: 15, color: AppColors.warmGray)),
                const SizedBox(height: 32),

                _Field(label: 'Full Name', hint: 'Jane Doe', controller: _name,
                    validator: (v) => (v?.trim().isEmpty ?? true) ? 'Enter your name' : null),
                const SizedBox(height: 16),
                _Field(label: 'Email', hint: 'jane@email.com', controller: _email,
                    keyboardType: TextInputType.emailAddress,
                    validator: (v) => (v?.contains('@') ?? false) ? null : 'Enter a valid email'),
                const SizedBox(height: 16),
                _Field(label: 'Phone (optional)', hint: '08012345678', controller: _phone,
                    keyboardType: TextInputType.phone),
                const SizedBox(height: 16),
                _Field(
                  label: 'Password', hint: '••••••••', controller: _password,
                  obscure: _obscure,
                  suffix: IconButton(
                ...
                Row(
                  ...
                  children: [
                    Checkbox(
                      value: _agreed, activeColor: AppColors.coral,
...
class _LoginScreenState extends State<LoginScreen> {
            ...
            child: Column(
              ...
              children: [
                const Text('Welcome back!',
                    style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.darkText)),
                const SizedBox(height: 6),
                Text('Log in to your DesiredWash account',
                    style: TextStyle(fontSize: 15, color: AppColors.warmGray)),
                const SizedBox(height: 32),
                ...
                _Field(
                  ...
                  validator: (v) => (v?.isNotEmpty ?? false) ? null : 'Enter your password',
                ),
  ...
  Future<void> _submit() async {
    ...
    if (ok && mounted) Navigator.pushNamedAndRemoveUntil(context, '/', (_) => false);
  }
}
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Container(
                        width: 80, height: 80,
                        decoration: BoxDecoration(color: AppColors.peach.withOpacity(0.3), shape: BoxShape.circle),
                        child: const Icon(Icons.mark_email_read_outlined, color: AppColors.coral, size: 40),
                      ),
                      const SizedBox(height: 20),
                      ...
                      Text('Reset link sent to your email.',
                          style: TextStyle(color: AppColors.warmGray), textAlign: TextAlign.center),
                      const SizedBox(height: 28),
                      GestureDetector(
...
class _Field extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.darkText)),
      const SizedBox(height: 8),
      TextFormField(
...
class _SocialButton extends StatelessWidget {
  ...
  final bool dark;
  const _SocialButton({required this.label, required this.icon, required this.onTap, this.dark = false});
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: OutlinedButton(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(icon, color: dark ? Colors.white : AppColors.darkText, size: 22),
