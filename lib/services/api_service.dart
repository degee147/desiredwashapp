...
class ApiService {
  static const String baseUrl = 'https://api.desiredwash.com/v1';
  ...
  Future<Map<String, dynamic>> _post(String path, Map<String, dynamic> body) async {
    ...
    if (res.statusCode >= 400) throw ApiException(data['message'] ?? 'Server error', res.statusCode);
    return data;
  }
  ...
  Future<AppUser> updateProfile({String? name, String? phone, String? zoneId}) async {
  ...
  Future<CreateOrderResult> createOrder({
    ...
    final data = await _post('/orders', {
      ...
      'scheduled_pickup_date': scheduledPickupDate.toIso8601String().split('T').first,
  ...
  Future<List<PickupOrder>> getOrders() async {
    ...
    return (data['orders'] as List).map((e) => PickupOrder.fromJson(e)).toList();
  }
  ...
  Future<double> verifyWalletTopup(String transactionRef) async {
    final data = await _post('/wallet/topup/verify', {'transaction_ref': transactionRef});
...
class PaymentVerificationResult {
  ...
  PaymentVerificationResult({required this.success, required this.message, this.order});

  factory PaymentVerificationResult.fromJson(Map<String, dynamic> json) => PaymentVerificationResult(
        success: json['success'],
        message: json['message'],
        order: json['order'] != null ? PickupOrder.fromJson(json['order']) : null,
      );
}
